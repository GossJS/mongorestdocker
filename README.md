...
(если есть домен, например, ikoder.xyz, то оперативно изменяем его DNS-зону с учётом полученного IP)
...
А по адресу http://95.213.199.198/alldocs/templates?apiKey=ifna212ASFisfsjaAFFF

или http://ikoder.xyz/alldocs/templates?apiKey=ifna212ASFisfsjaAFFF
...
В файле default.conf используются имена, которые docker помещает в файл hosts - это имена контейнеров. Без них пришлось бы использовать захардкоденные адреса (ikodder.xyz вместо 
